								...
								<tr @if( $r->status == App\Model\Reservation::REJECTED ) class="bg-danger" @endif>
									...
									<td>
										@if( $r->status == App\Model\Reservation::PENDING )
											<a href="{{ route('reservation.approve', ['reservation_id' => $r->id]) }}" class="btn btn-success btn-xs" title="Approve">
												<i class="glyphicon glyphicon-check"></i> Approve 
											</a> &nbsp; 
											<a href="{{ route('reservation.reject', ['reservation_id' => $r->id]) }}" class="btn btn-danger btn-xs" title="Reject">
